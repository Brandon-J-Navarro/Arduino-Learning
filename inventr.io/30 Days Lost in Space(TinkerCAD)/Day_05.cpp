...
void loop()
{
  if (digitalRead(CABIN_LIGHTS_SWITCH_PIN) == HIGH && digitalRead(STORAGE_LIGHTS_PIN) == LOW && digitalRead(COCKPIT_LIGHTS_PIN) == LOW){
    digitalWrite(CABIN_LIGHTS_PIN, HIGH);
  } else {
    digitalWrite(CABIN_LIGHTS_PIN, LOW);
  }
 
  if (digitalRead(STORAGE_LIGHTS_SWITCH_PIN) == HIGH && digitalRead(CABIN_LIGHTS_PIN) == LOW && digitalRead(COCKPIT_LIGHTS_PIN) == LOW){
    digitalWrite(STORAGE_LIGHTS_PIN, HIGH);
  } else {
    digitalWrite(STORAGE_LIGHTS_PIN, LOW);
  }
  
  if (digitalRead(COCKPIT_LIGHTS_SWITCH_PIN) == HIGH && digitalRead(STORAGE_LIGHTS_PIN) == LOW && digitalRead(CABIN_LIGHTS_PIN) == LOW){
